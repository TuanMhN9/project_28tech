...
-- SELECT *  FROM ((((building INNER JOIN district ON building.id = district.id)
-- INNER JOIN assignmentbuilding ON building.id = assignmentbuilding.buildingid)
-- INNER JOIN buildingrenttype ON building.id = buildingrenttype.buildingid)
-- INNER JOIN renttype ON renttype.id = buildingrenttype.renttypeid)
-- Where building.name LIKE '%building%' AND building.floorarea =500 AND building.id =1 AND building.ward LIKE '%2%' AND building.street LIKE '%long%' AND building.numberofbasement=2 AND building.floorarea >=200 AND building.floorarea <=500 AND building.rentprice >=15 AND building.rentprice <=20 AND assignmentbuilding.staffid = 3 AND (renttype.id = 1 OR renttype.id = 2 OR renttype.id = 3 );
